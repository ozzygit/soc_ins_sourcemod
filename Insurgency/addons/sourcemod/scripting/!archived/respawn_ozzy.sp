...
//new g_cqcMapsArray[][] = {"chateau_tunnels_v2","congress_coop","cargo_redux9","game_day_coopv1_3", - commented by oz
//				"launch_control_coopv1_5","bunker_busting_coop_ws","gizab_b1_coop", - commented by oz
//				"inferno_checkpoint_b2","nightfall_coopv1_1","caves_coop"}; - commented by oz
...
//new Handle:g_donorTagRemove_Array; - commented by oz
...
new
	...
	Event_RoundEnd,
	Event_RoundEnd_Pre,
	Event_ObjectDestroyed_pre,
	Event_ObjectDestroyed,
	Event_ObjectDestroyed_Post,
	Event_ControlPointCaptured,
	Event_ControlPointCaptured_Pre,
	Event_ControlPointCaptured_Post,
	Event_ObjectDestroyed_Pre,
	Event_PlayerDisconnect,
	Event_PlayerConnect,
	Event_PlayerReload_Pre,
	Event_OnFlashPlayerPre,

	//Dynamic Respawn cvars 
	Float:g_DynamicRespawn_Distance_mult, //daimyo original code had no Float: at the beginning causing tag mismatch
	...
	//g_maxbots_std = 0, - commented out by oz - symbol never used?
	//g_maxbots_light = 0, - commented out by oz - symbol never used?
	//g_maxbots_jug = 0, - commented out by oz - symbol never used?
	//g_maxbots_bomb = 0, - commented out by oz - symbol never used?
	//Template of bots AI Director uses
	//g_bots_std = 0, - commented out by oz
	//g_bots_light = 0, - commented out by oz
	//g_bots_jug = 0, - commented out by oz
	//g_bots_bomb = 0, - commented out by oz
...
{	
	...
	Dynamic_Loadouts(); - commented out by oz
...
public Action:Timer_MapStart(Handle:Timer)
{
	...
	if (StrEqual(sGameMode,"hunt")) // if Hunt? - commented out by oz
	{ 
		g_isHunt = 1;
		g_iCvar_SpawnMode = 0;
	...
	if (g_isConquer != 1 && g_isOutpost != 1) 
		CreateTimer(30, Timer_Enemies_Remaining,_ , TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
...
public Action:Timer_EliteBots(Handle:Timer)
{
	...
	if (Ins_InCounterAttack())
	{
		...
		if ((acp+1) == ncp)
		{
			//PrintToServer("ENABLE ELITE FINALE"); // commented by oz
			g_isEliteCounter = 1;
			//EnableDisableEliteBotCvars(1, 1);
		}
		else
		{
			//PrintToServer("ENABLE ELITE NORMAL"); // commented by oz
			g_isEliteCounter = 1;
...
public Action:Timer_WaveSpawning(Handle:Timer)
{
	...
	if (g_respawn_mode_team_sec)
	{
		...
		{
			//decl String:textToPrintChat[64];
			//decl String:textToPrint[64]; - commented out by oz
			...
			//PrintHintTextToAll(textToPrint); - commented by oz
			//PrintToChatAll(textToPrintChat); - commented by oz
		...
		{
			//PrintHintTextToAll("[INTEL]Team Reinforced!"); - commented by oz
			//PrintToChatAll("[INTEL]Team Reinforced!"); - commented by oz
		...
		public Action:Timer_Enemies_Remaining(Handle:timer)
{
	...
	if (validAntenna != -1 || g_jammerRequired == 0)
	{
		...
		decl String:textToPrintChat[64];
		//decl String:textToPrint[64];
		//Format(textToPrintChat, sizeof(textToPrintChat), "[INTEL]Enemies alive: %d | Enemy reinforcements left: %d", alive_insurgents, g_iRemaining_lives_team_ins); - commented by oz
		Format(textToPrintChat, sizeof(textToPrintChat), "[INTEL]Enemies alive: %d", alive_insurgents, g_iRemaining_lives_team_ins);
		//Format(textToPrint, sizeof(textToPrint), "[INTEL]Enemies alive: %d | Enemy reinforcements left: %d", alive_insurgents ,g_iRemaining_lives_team_ins); - commented by oz
		//PrintHintTextToAll(textToPrint); - commented by oz
		PrintToChatAll(textToPrintChat);
		...
		new jammerSpawnReductionAmt = (g_iRespawnSeconds / timeReduce);
		//Format(textToPrintChat, sizeof(textToPrintChat), "[INTEL]Jammer Active! | Reinforce time reduced by: %d seconds", jammerSpawnReductionAmt); - commented out by oz
		PrintToChatAll(textToPrintChat);
	...
	else
	{
		...
		PrintHintTextToAll(textToPrint);
		PrintToChatAll(textToPrintChat);
		//Format(textToPrintChat, sizeof(textToPrintChat), "[INTEL]Jammer Inactive! | Reinforce time reduced by: %d seconds", 0); - commented out by oz
		PrintToChatAll(textToPrintChat);
	}
...
void AI_Director_RandomEnemyReinforce()
{
		...
		if (g_iRemaining_lives_team_ins > 0)
	{
		...
		// {
			...
			//PrintToServer("g_iReinforceTime %d, Ambush Reinforcements added to existing!",g_iReinforceTime); // commented by oz
			// if (g_isHunt == 1)
	...
	else
	{
		...
		g_iReinforceTime = g_iReinforceTimeSubsequent_AD_Temp;
		//PrintToServer("g_iReinforceTime %d, AMBUSH Reinforcements Arrived Normally!",g_iReinforceTime); // commented by oz
		...
		if (g_iReinforceTime % 10 == 0 && g_iReinforceTime > 10)
		{
			...
			if (validAntenna != -1 || g_jammerRequired == 0)
			{
				...
				else
				{
					...
					Format(textToPrint, sizeof(textToPrint), "[INTEL]Enemies reinforce in %d seconds", g_iReinforceTime);
		...
		if (g_iReinforceTime <= 0)
		{
			...
			if (g_iRemaining_lives_team_ins > 0)
			{
				...
				if (g_iRemaining_lives_team_ins < (g_iRespawn_lives_team_ins / g_iReinforce_Mult) + g_iReinforce_Mult_Base)
				{
					...
					g_iReinforceTime = g_iReinforceTimeSubsequent_AD_Temp;
					//PrintToServer("g_iReinforceTime %d, Reinforcements added to existing!",g_iReinforceTime); // commented by oz
					if (g_isHunt == 1)
						 g_iReinforceTime = g_iReinforceTimeSubsequent_AD_Temp * g_iReinforce_Mult;
					//if (g_huntCacheDestroyed == true && g_isHunt == 1)
					//	 g_iReinforceTime = g_iReinforceTime + g_huntReinforceCacheAdd;
				...
				else
				{
					...
					//PrintToServer("g_iReinforceTime %d, Reinforcements max capacity!",g_iReinforceTime); // commented by oz
					if (g_isHunt == 1)
			...
			else
			{
				...
				g_iReinforceTime = g_iReinforceTimeSubsequent_AD_Temp;
				//PrintToServer("g_iReinforceTime %d, Reinforcements Arrived Normally!",g_iReinforceTime); // commented by oz
				...
				if (IsClientInGame(enemyBot) && IsFakeClient(enemyBot))
				{
					...
					if (IsPlayerAlive(enemyBot) && m_iTeam == TEAM_2_INS)
					{
						...
						if (tDistance <= 150 && Check_NearbyPlayers(enemyBot) && (capDistance > 800 || g_botStaticGlobal[enemyBot] > 120)) 
						{
							...
							//PrintToServer("ENEMY STATIC - KILLING"); // commented by oz
...
//commented out by oz - sernix specific theater?
...
// commented out by oz - Symbol GetPlayerAmmo never used
...
public Action:Event_PlayerReload_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//new m_iTeam = GetClientTeam(client); - commented by oz
...
//Ins_InCounterAttack
	...
	//new Float:tBadPos[3]; - commented out by oz
	...
	//  	}
	//  }		
	//PrintToServer("CHECKSPAWN | m_nActivePushPointIndex: %d",m_nActivePushPointIndex); // commented by oz
	return 1;
} 
...
float GetSpawnPoint_SpawnPoint(client) {
	...
	//PrintToServer("1st Pass: Could not find acceptable ins_spawnzone for %N (%d)", client, client); // commented by oz
	...
	//PrintToServer("2nd Pass: Could not find acceptable ins_spawnzone for %N (%d)", client, client); // commented by oz
	//Lets try again but wider range
	...
	while (point3 != -1) {
		...
		//if (m_iTeamNum == m_iTeam) {
			...
			if (CheckSpawnPoint(vecSpawn,client,tObjectiveDistance,m_nActivePushPointIndex)) {
				...
				//new m_nActivePushPointIndex = Ins_ObjectiveResource_GetProp("m_nActivePushPointIndex");
				//PrintToServer("FOUND! m_nActivePushPointIndex: %d %N (%d) spawnpoint %d Distance: %f tObjectiveDistance: %f g_flMinObjectiveDistance %f RAW ACP: %d",m_nActivePushPointIndex, client, client, point3, distance, tObjectiveDistance, g_flMinObjectiveDistance, acp); // commented by oz
				return vecSpawn;
			}
			else
			{
	...
	//PrintToServer("3rd Pass: Could not find acceptable ins_spawnzone for %N (%d)", client, client); // commented by oz
	...
	while (pointFinal != -1) {
		...
		//if (m_iTeamNum == m_iTeam) {
			...
			Ins_ObjectiveResource_GetPropVector("m_vCPPositions",m_vCPPositions[m_nActivePushPointIndex],m_nActivePushPointIndex);
			//distance = GetVectorDistance(vecSpawn,m_vCPPositions[m_nActivePushPointIndex]);- commented out by oz
			if (CheckSpawnPoint(vecSpawn,client,tObjectiveDistance,m_nActivePushPointIndex)) {
				vecSpawn = GetInsSpawnGround(pointFinal, vecSpawn);
				//new m_nActivePushPointIndex = Ins_ObjectiveResource_GetProp("m_nActivePushPointIndex");
				//PrintToServer("FINAL PASS FOUND! m_nActivePushPointIndex: %d %N (%d) spawnpoint %d Distance: %f tObjectiveDistance: %f g_flMinObjectiveDistance: %f RAW ACP: %d",m_nActivePushPointIndex, client, client, pointFinal, distance, tObjectiveDistance, g_flMinObjectiveDistance, acp); // commented by oz
				return vecSpawn;
			}
			else
			{
	...
	//PrintToServer("Final Pass: Could not find acceptable ins_spawnzone for %N (%d)", client, client); // commented by oz
	return vecOrigin;
}
...
public Action:Event_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_playerFirstJoin[client] == 1 && !IsFakeClient(client))
	{
		...
		if (isPlayerNew == -1)
		{
			...
			//PrintToServer("SPAWN: Player %N is new! | SteamID: %s | PlayerArrayList Size: %d", client, steamId, GetArraySize(g_playerArrayList)); // commented by oz
		}
	}
...
public Action:Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//commented out by oz - strings not used
	//g_maxbots_std = 0;
	//g_maxbots_light = 0;
	//g_maxbots_jug = 0;
	//g_maxbots_bomb = 0;

	//g_bots_std = 0; - commented by oz
	//g_bots_light = 0; - commented by oz
	//g_bots_jug = 0; - commented by oz
	//g_bots_bomb = 0; - commented by oz
	////////////////////////
	// Rank System
	...
	while ((ent = FindEntityByClassname(ent, "healthkit")) > MaxClients && IsValidEntity(ent))
	{
		...
		//PrintToServer("KILL HEALTHKITS"); // commented by oz
		AcceptEntityInput(ent, "Kill");
	}
...
public Action:Event_ControlPointCaptured_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new Float:fRandom = GetRandomFloat(0.0, 1.0);
	//PrintToServer("Counter Chance = %f", g_respawn_counter_chance); // commented by oz
	// Occurs counter attack
	if (fRandom < g_respawn_counter_chance && g_isCheckpoint == 1 && ((acp+1) != ncp))
	{
		...
		if (largeCounterEnabled)
		{
			...
			//PrintToChatAll("[INTEL]: Enemy forces are sending a large counter-attack your way!  Get ready to defend!"); - commented out by oz
		}
...
public Action:Event_ObjectDestroyed_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (fRandom < g_respawn_counter_chance && g_isCheckpoint && ((acp+1) != ncp))
	{
		...
		if (largeCounterEnabled)
		{
			//PrintHintTextToAll("[INTEL]: Enemy forces are sending a large counter-attack your way!  Get ready to defend!"); - commented out by oz
			//PrintToChatAll("[INTEL]: Enemy forces are sending a large counter-attack your way!  Get ready to defend!"); - commented out by oz
		}
		g_AIDir_TeamStatus -= 5;
		// Call music timer
...
public Action:Event_ObjectDestroyed(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (g_isHunt == 1)
	{
		//g_huntCacheDestroyed = true; - commented out by oz
		//g_iReinforceTime = g_iReinforceTime + g_huntReinforceCacheAdd;
...
void EnableDisableEliteBotCvars(tEnabled, isFinale)
{
	...
	if (tEnabled == 1)
	{
		//PrintToServer("BOT_SETTINGS_APPLIED"); // commented by oz
		if (isFinale == 1)
		{
	...
	{
		//PrintToServer("BOT_SETTINGS_APPLIED_2"); // commented by oz
...
// Below void commented out by oz
// Stop counter-attack music
...
void ResetSecurityLives()
{
	...
	//if (g_isConquer == 1 || g_isOutpost == 1) return;
		// The number of control points
	//new ncp = Ins_ObjectiveResource_GetProp("m_iNumControlPoints"); - commented out by oz
	// Active control poin
	//new acp = Ins_ObjectiveResource_GetProp("m_nActivePushPointIndex"); - commented out by oz
...
public Action:Event_PlayerPickSquad_Post( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	if (g_iRoundStatus == 1 && g_playerFirstJoin[client] == 1 && !IsPlayerAlive(client) && team == TEAM_1_SEC)
	{
		...
		if (isPlayerNew != -1)
		{
			//PrintToServer("Player %N has reconnected! | SteamID: %s | Index: %d", client, steamId, isPlayerNew); // commented by oz
		}
		else
		{
			PushArrayString(g_playerArrayList, steamId);
			//PrintToServer("Player %N is new! | SteamID: %s | PlayerArrayList Size: %d", client, steamId, GetArraySize(g_playerArrayList)); // commented by oz
		...
		if ((team == TEAM_1_SEC) || (team == TEAM_2_INS))
		{
			...
			else if ((g_iCvar_respawn_type_team_sec == 1 && team == TEAM_1_SEC) || (g_iCvar_respawn_type_team_ins == 1 && team == TEAM_2_INS))
			{
				...
				else if (g_iSpawnTokens[client] <= 0 && g_iRespawnCount[team] > 0)
				{
					...
					decl String:sChat[128];
					Format(sChat, 128,"You cannot be respawned anymore. (out of lives)");
					//PrintToChat(client, "%s", sChat); - commented out by oz
				}
			}
			// Respawn type 2 for players
	...
	if (g_fCvar_fatal_chance > 0.0)
		{
			if (g_iHurtFatal[client] == 1 && !IsFakeClient(client))
			{
				...
				//PrintHintText(client, "%s", fatal_hint); - commented out by oz
				//PrintToChat(client, "%s", fatal_hint); - commented out by oz
			}
			else
			{
				Format(wound_hint, 255,"You're %s for %i damage, call a medic for revive!", woundType, g_clientDamageDone[client]);
				//PrintHintText(client, "%s", wound_hint); - commented out by oz
				//PrintToChat(client, "%s", wound_hint); - commented out by oz
			}
		}
		else
		{
...
{	
	...
	{
		new fRandomFloat = GetRandomFloat(0.0, 1.0);
		...
		if (g_cqc_map_enabled == 1 && Ins_InCounterAttack())
		{
			if (StrContains(g_client_last_classstring[client], "bomber") > -1)
			{
				//PrintToServer("BOMBER SPAWN: Delay %f", (g_fCvar_respawn_delay_team_ins_spec / 3)); // commented by oz
				CreateTimer((g_fCvar_respawn_delay_team_ins_spec / 3), RespawnBot, client);
			}
			else if (StrContains(g_client_last_classstring[client], "juggernaut") > -1)
			{
				//PrintToServer("JUGGER SPAWN: Delay %f", (g_fCvar_respawn_delay_team_ins_spec / 4)); // commented by oz
...
//Funcion commented out by oz but body already commented out 
...
public Action:Timer_PlayerRespawn(Handle:Timer, any:client)
{
	...
	if (!IsPlayerAlive(client) && g_iRoundStatus == 1)
	{
		...
		{	
			if (g_playerFirstJoin[client] == 1)
			{
				...
				if (!IsFakeClient(client))
				{
					...
					//PrintCenterText(client, sRemainingTime); - commented out by oz
				}
			}
			else
			{
		...
		else
		{
			...
			// Print remaining time to center text area
			//if (!IsFakeClient(client)) - commented out by oz
			//	PrintCenterText(client, "You reinforced! (%d lives left)", g_iSpawnTokens[client]); - commented out by OnClientAuthorized
		...
		else if (iTeam == TEAM_1_SEC && IsPlayerAlive(medic) && !(StrContains(g_client_last_classstring[medic], "medic") > -1))
		{
			...
			if ((StrContains(checkWeapon, "weapon_healthkit") > -1))
			{
				...
				else //Heal Self
				{
					...
					// Check weapon
					//new ActiveWeapon = GetEntPropEnt(medic, Prop_Data, "m_hActiveWeapon"); - variable already defined above
					if (ActiveWeapon < 0)
						continue;
					decl String:sWeapon[32];
					GetEdictClassname(ActiveWeapon, sWeapon, sizeof(sWeapon));
...
public Action:Timer_VIPCheck_Main(Handle:timer, any:data)
{
	...
	{				
		...
		//PrintToChatAll("Destroyable objectives should work as well.");- commented out by oz
		//PrintHintTextToAll(textToPrint);
		//PrintToChatAll(textToPrintChat); - commented out by oz
		if (g_nVIP_ID == 0)
		{
	...
	{
		g_vip_obj_ready = 0;
		//PrintToChatAll("\x04VIP\x01 failed to capture point within allotted time of %d seconds.", g_iCvar_vip_obj_time); -  commented out by oz
...
// 		//PrintToServer("[AI_DIRECTOR] STATUS: %i | g_AIDir_CurrDiff %d | InCounter: %d | DiffChanceBase: %d", g_AIDir_TeamStatus, g_AIDir_CurrDiff, tIsInCounter, g_AIDir_DiffChanceBase); // commented by oz
// 		//PrintToServer("[AI_DIRECTOR]: Ambush_Counter: %d | tAmbushChance: %d | AmbushCond_Chance %d",g_AIDir_AmbushCond_Counter, tAmbushChance, g_AIDir_AmbushCond_Chance); // commented by oz
// 		//PrintToServer("[AI_DIRECTOR]: AmbushCond_Chance: %d | ChangeCond_Counter: %d | ChangeCond_Rand %d",g_AIDir_AmbushCond_Rand, g_AIDir_ChangeCond_Counter, g_AIDir_ChangeCond_Rand); // commented by oz
...
// 		//PrintToServer("[AI_DIRECTOR] STATUS: %i | SetDifficulty CALLED", g_AIDir_TeamStatus); // commented by oz
...
// 		else
// 		{
// 			//PrintToServer("[AI_DIRECTOR]: tAmbushChance: %d | g_AIDir_AmbushCond_Chance %d", tAmbushChance, g_AIDir_AmbushCond_Chance); // commented by oz
// 			//Reset
// 			g_AIDir_AmbushCond_Counter = 0;
// 			g_AIDir_AmbushCond_Rand = GetRandomInt(g_AIDir_AmbushCond_Min, g_AIDir_AmbushCond_Max);
// 		}
				...
				if (IsClientInGame(client) && IsPlayerAlive(client) && GetClientTeam(client) == 2)
				{
					...
					else if ((StrContains(g_client_last_classstring[client], "medic") > -1))
					{
						 ...
						 if (GetVectorDistance(fPlayerOrigin, fOrigin) <= Healthkit_Radius)
						{
							...
							decl String:sWeapon[32];
							GetEdictClassname(ActiveWeapon, sWeapon, sizeof(sWeapon));
							//new bool:bCanHealPaddle = false; - commented by oz
							if (((StrContains(sWeapon, "weapon_defib") > -1) || (StrContains(sWeapon, "weapon_knife") > -1) || (StrContains(sWeapon, "weapon_kabar") > -1)))
							{
								//PrintToServer("DEBUG 3");
								new iHealth = GetClientHealth(client);
...
public AI_Director_SetDifficulty(g_AIDir_TeamStatus, g_AIDir_TeamStatus_max)
{
	...
	new 
		//AID_ReinfAdj_low = 10, AID_ReinfAdj_med = 20, AID_ReinfAdj_high = 30, AID_ReinfAdj_pScale = 0, - commented out by oz
	...
	if (fRandomInt <= (g_AIDir_DiffChanceBase + AID_SetDiffChance_pScale))
	{
		...
		//g_AIDir_CurrDiff = 5; - commented out by oz
	...
	else if (g_AIDir_TeamStatus < (g_AIDir_TeamStatus_max / 4))
	{
		...
		//DEBUG: Track Current Difficulty setting
		//g_AIDir_CurrDiff = 1; - commented out by oz
	...
	else if (g_AIDir_TeamStatus >= (g_AIDir_TeamStatus_max / 4) && g_AIDir_TeamStatus < (g_AIDir_TeamStatus_max / 2))
	{
		...
		if (g_AIDir_TeamStatus >= (g_AIDir_TeamStatus_max / 4) && g_AIDir_TeamStatus < (g_AIDir_TeamStatus_max / 3) && GetTeamSecCount() <= 9)
		{
			...
			//DEBUG: Track Current Difficulty setting
			//g_AIDir_CurrDiff = 2; - commented out by oz
	...
	else if (g_AIDir_TeamStatus >= (g_AIDir_TeamStatus_max / 2) && g_AIDir_TeamStatus < ((g_AIDir_TeamStatus_max / 4) * 3))
	{
		...
		//DEBUG: Track Current Difficulty setting
		//g_AIDir_CurrDiff = 3; - commented out by oz
	...
	else if (g_AIDir_TeamStatus >= ((g_AIDir_TeamStatus_max / 4) * 3))
	{
		...
		//DEBUG: Track Current Difficulty setting
		//g_AIDir_CurrDiff = 4; - commented by oz
...
//OnButtonRelease(client, button) - commented out by oz
//{ - commented out by oz
