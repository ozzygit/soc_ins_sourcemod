...
//Ins_InCounterAttack
	...
	for (new iTarget = 1; iTarget < MaxClients; iTarget++) 
		...
		if (!IsPlayerAlive(iTarget)) continue;
		//if (!IsClientInGame(iTarget))     continue;
		//if (!IsPlayerAlive(iTarget))      continue;
...
public Action:Event_ControlPointCaptured_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ((acp+1) == ncp)
	{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			...
			{
				if(IsFakeClient(i)) ForcePlayerSuicide(i);
			}
		}
...
public Action:FireScreamCheckTimer(Handle:timer, any:entity)
{
	...
	for (new client = 1;client <= MaxClients;client++)
	{
		...
		if (owner <= 0 || !IsClientInGame(owner) || !IsClientConnected(owner) || IsFakeClient(owner)) continue;
