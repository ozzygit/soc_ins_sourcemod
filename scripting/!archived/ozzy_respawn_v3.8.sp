...
//	g_jammerRequired = GetConVarInt(sm_jammer_required);
	...
	//Dynamic Respawns
	Float:g_DynamicRespawn_Distance_mult = GetConVarFloat(sm_respawn_dynamic_distance_multiplier);
...
public Action:Timer_MapStart(Handle:Timer)
{
	...
	Float:g_bot_attack_aimpenalty_time_far_mult = 2.0;
	...
	Float:g_bot_attack_aimpenalty_time_close_org = GetConVarFloat(FindConVar("bot_attack_aimpenalty_time_close"));
	Float:g_bot_attack_aimpenalty_time_far_org = GetConVarFloat(FindConVar("bot_attack_aimpenalty_time_far"));
	Float:g_bot_attack_aimtolerance_newthreat_amt_org = GetConVarFloat(FindConVar("bot_attack_aimtolerance_newthreat_amt"));
	Float:g_bot_attackdelay_frac_difficulty_impossible_org = GetConVarFloat(FindConVar("bot_attackdelay_frac_difficulty_impossible"));
	Float:g_bot_aim_aimtracking_base_org = GetConVarFloat(FindConVar("bot_aim_aimtracking_base"));
	Float:g_bot_aim_aimtracking_frac_impossible_org = GetConVarFloat(FindConVar("bot_aim_aimtracking_frac_impossible"));
	Float:g_bot_aim_angularvelocity_frac_impossible_org = GetConVarFloat(FindConVar("bot_aim_angularvelocity_frac_impossible"));
	Float:g_bot_aim_angularvelocity_frac_sprinting_target_org = GetConVarFloat(FindConVar("bot_aim_angularvelocity_frac_sprinting_target"));
	Float:g_bot_aim_attack_aimtolerance_frac_impossible_org = GetConVarFloat(FindConVar("bot_aim_attack_aimtolerance_frac_impossible"));
...
void AI_Director_RandomEnemyReinforce()
{
	...
	if (g_iRemaining_lives_team_ins > 0)
	{
		// if (g_iRemaining_lives_team_ins < (g_iRespawn_lives_team_ins / g_iReinforce_Mult) + g_iReinforce_Mult_Base)
			...
			Format(textToPrint, sizeof(textToPrint), "[INTEL]ISIS Reinforcements incoming!");
	...
	else
	{
		...
		Format(textToPrint, sizeof(textToPrint), "[INTEL]ISIS ambush arriving!");
...
//	if ((0 >= client || client > MaxClients) || !IsClientInGame(client)) return 0;
	...
	if ((((acp+1) == ncp) || Ins_InCounterAttack()) && fRandomInt <= 10)
	{
		...
		Float:distance = GetVectorDistance(vecSpawn,m_vCPPositions[m_nActivePushPointIndexFinal]);
...
CheckSpawnPointPlayers(Float:vecSpawn[3],client, Float:tObjectiveDistance) 
{
	...
	if ((((acp+1) == ncp) || Ins_InCounterAttack()) && fRandomInt < 25)
	{
		...
		m_nActivePushPointIndexFinal -= 1;
		Float:objDistance = GetVectorDistance(vecSpawn,m_vCPPositions[m_nActivePushPointIndexFinal]);
...
public Action:Event_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_playerFirstJoin[client] == 1 && !IsFakeClient(client))
	{
		...
		decl String:steamId[64]; //GetClientAuthString(client, steamId, sizeof(steamId));
...
public Action:Event_ControlPointCaptured_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new ncp = Ins_ObjectiveResource_GetProp("m_iNumControlPoints"); // Get the number of control points
	new acp = Ins_ObjectiveResource_GetProp("m_nActivePushPointIndex"); // Get active push point
	...
	else if (((acp+1) == ncp))
	{
		...
		//Create Counter End Timer
		g_isEliteCounter = 1;
		CreateTimer((Float:cvar_ca_dur + 1.0), Timer_CounterAttackEnd);
...
public Action:Timer_PlayerRespawn(Handle:Timer, any:client)
{
    ...
    if (!IsPlayerAlive(client) && g_iRoundStatus == 1)
    {
        ...
        else
        {
            ...
            new tSquadSpawned = false;             //Lets confirm squad spawn
            new playerRag = EntRefToEntIndex(g_iClientRagdolls[client]); // Get ragdoll position
            ...
            if (!g_respawn_mode_team_sec)
            {
            if (g_squadSpawnEnabled[client] == 1 && bool:tSquadSpawned == true)
	...
	for (new iMedic = 1; iMedic <= MaxClients; iMedic++)
	{
		...
		else if (IsPlayerAlive(iMedic) && !(StrContains(g_client_last_classstring[iMedic], "medic") > -1))
		{
			...
			)
			{
				...
				if (fDistance < fReviveDistance && (ClientCanSeeVector(iMedic, fRagPos, fReviveDistance))
					&& ((StrContains(sWeapon, "healthkit") > -1)))
				{
				...
				else if (bCanHealMedpack)
				{
					if (iHealth < 100)
					{
						...
						if (g_playerMedicHealsAccumulated[medic] >= iHealthCap && iRewardMedicEnabled == 1)
						{
							...
							//decl String:sBuf2[255];
							// if (iBonus > 1)
							// 	Format(sBuf2, 255,"Awarded %i kills for healing %i in HP of other players.", iBonus, i4thCap);
							// else
							//Format(sBuf2, 255,"Awarded %i life for healing %i in HP of other players.", 1, iHealthCap);
	...
	for (new friendlyMedic = 1; friendlyMedic <= MaxClients; friendlyMedic++)
	{
		if (IsClientConnected(friendlyMedic) && IsClientInGame(friendlyMedic) && !IsFakeClient(friendlyMedic))
		{
			...
			//PrintToServer("Medic 3");
				new bool:bCanHealPaddle = false;
				if ((StrContains(sWeapon, "weapon_defib") > -1) || (StrContains(sWeapon, "weapon_knife") > -1) || (StrContains(sWeapon, "weapon_kabar") > -1) || (StrContains(sWeapon, "healthkit") > -1))
				{
